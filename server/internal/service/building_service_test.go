...
func TestStartUpgrade_Success(t *testing.T) {
	...
	// Upgrade food_1 from level 0 to level 1 (no prerequisites, affordable)
	resp, err := svc.StartUpgrade(ctx, playerID, villageID, "food_1")
	if err != nil {
		t.Fatalf("expected success, got error: %v", err)
	}

	if resp.BuildingType != "food_1" {
		t.Errorf("expected building_type food_1, got %s", resp.BuildingType)
	}
...
func TestStartUpgrade_InsufficientResources(t *testing.T) {
	...
	// Town Hall costs Food:100, Water:200, Lumber:200, Stone:200 at level 1.
	...
	// After spending F:100 W:200 L:200 S:200, resources are now F:400 W:300 L:300 S:300.
	// Try town_hall again (F:100 W:200 L:200 S:200) — should still work
	_, err = svc.StartUpgrade(ctx, playerID, villageID, "town_hall")
	if err != nil {
	...
	// Now resources are F:300 W:100 L:100 S:100 — town_hall costs F:100 W:200 L:200 S:200 → should fail (Water/Lumber/Stone too low)
	_, err = svc.StartUpgrade(ctx, playerID, villageID, "town_hall")
	if err == nil {
...
func TestStartUpgrade_BuildingInProgress(t *testing.T) {
	...
	// Start first upgrade
	_, err := svc.StartUpgrade(ctx, playerID, villageID, "food_1")
	if err != nil {
		t.Fatalf("first upgrade should succeed: %v", err)
	}

	// Try to start second upgrade — should fail
	_, err = svc.StartUpgrade(ctx, playerID, villageID, "lumber_1")
	if err == nil {
...
func TestStartUpgrade_NotOwner(t *testing.T) {
	...
	_, err := svc.StartUpgrade(ctx, 99999, villageID, "food_1")
	if err == nil {
...
func TestCompleteBuilds_PromotesLevel(t *testing.T) {
	...
	for _, b := range buildings {
		if b.BuildingType == "food_1" {
			if b.Level != 1 {
				t.Errorf("expected food_1 level 1, got %d", b.Level)
			}
			break
		}
	}
...
func TestCompleteBuilds_UpdatesResourceRates(t *testing.T) {
	...
	// Insert a completed queue item for food_1 → level 1
	pastItem := &model.BuildingQueue{
		VillageID:    villageID,
		BuildingType: "food_1",
		TargetLevel:  1,
	...
	// Verify food rate was updated: base(1) + level(1) * rate_per_level(2) = 3
	res, _ := svc.resourceRepo.Get(ctx, villageID)
	expectedRate := 1.0 + 2.0*1 // BaseResourceRate + RatePerLevel * level
	if res.FoodRate != expectedRate {
		t.Errorf("expected food_rate %.1f, got %.1f", expectedRate, res.FoodRate)
	}
}
...
func TestGetUpgradeCost(t *testing.T) {
	...
	cost, err := svc.GetUpgradeCost(ctx, playerID, villageID, "food_1")
	if err != nil {
		t.Fatalf("expected success, got: %v", err)
	}

	if cost.BuildingType != "food_1" {
		t.Errorf("expected food_1, got %s", cost.BuildingType)
	}
